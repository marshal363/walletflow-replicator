      ...
      <Carousel className="w-full">
        <CarouselContent>
          {visibleActions.map((action) => (
            <CarouselItem key={action.id} className="basis-[85%] sm:basis-[45%] md:basis-[35%]">
              <div
                onClick={action.action}
                className={`relative flex-none h-[72px] overflow-hidden rounded-xl bg-gradient-to-r ${action.gradient} cursor-pointer transition-all hover:scale-[1.02]`}
              >
